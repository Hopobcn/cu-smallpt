...
__device__ inline
bool Intersect(const Sphere* d_spheres, std::size_t nb_spheres, const Ray& ray, std::size_t& id)
{
	bool hit = false;
	for (std::size_t i = 0u; i < nb_spheres; ++i)
	{
		if (d_spheres[i].Intersect(ray))
		{
			hit = true;
			id  = i;
		}
...
const Vector3 Radiance(const Sphere* d_spheres, std::size_t nb_spheres, const Ray& ray, RNG& rng)
{
	...
	{
		std::size_t id;
		const auto hit = Intersect(d_spheres, nb_spheres, r, id);
		if (!hit)
		{
			return L;
		}

		const Sphere& shape = d_spheres[id];
...
void kernel(const Sphere* d_spheres, std::size_t nb_spheres, Vector3* Ls, std::uint32_t w, std::uint32_t h, std::uint32_t nb_samples)
{
	...
	for (std::size_t y = blockIdx.y * blockDim.y + threadIdx.y; y < h; y += blockDim.y * gridDim.y)
	{
		for (std::size_t x = blockIdx.x * blockDim.x + threadIdx.x; x < w; x += blockDim.x * gridDim.x)
		{
...
void Render(std::uint32_t nb_samples) noexcept
{
	...
	Sphere* d_spheres;
	HANDLE_ERROR(cudaMalloc((void**)&d_spheres, sizeof(g_spheres)));
	HANDLE_ERROR(cudaMemcpy(d_spheres, g_spheres, sizeof(g_spheres), cudaMemcpyHostToDevice));
	Vector3* Ls_d;
	...
	kernel<<<nblocks, nthreads>>>(d_spheres, size(g_spheres), Ls_d, w, h, nb_samples);
	...
	HANDLE_ERROR(cudaMemcpy(Ls, Ls_d, nb_pixels * sizeof(Vector3), cudaMemcpyDeviceToHost));

	// Clean up device memory
	HANDLE_ERROR(cudaFree(Ls_d));
	HANDLE_ERROR(cudaFree(d_spheres));

	WritePPM(w, h, Ls, "cu-image.ppm");

	// Clean up host memory
	free(Ls);
}

} // end smallptr namespce
