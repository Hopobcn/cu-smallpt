...
static
void kernel(std::unique_ptr<Vector3[]>& Ls, std::uint32_t w, std::uint32_t h, std::uint32_t nb_samples)
{
	RNG rng;
...
void Render(std::uint32_t nb_samples) noexcept
{
	...
	std::unique_ptr<Vector3[]> Ls = std::make_unique<Vector3[]>(nb_pixels);

	kernel(Ls, w, h, nb_samples);

	WritePPM(w, h, Ls.get(), "seq-image.ppm");
}
